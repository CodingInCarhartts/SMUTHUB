...
export class BatotoParsers {
  ...
  static parseSearch(html: string, baseUrl: string): Manga[] {
    ...
    const potentialItems = root.querySelectorAll(
      'div.flex.border-b, div.group.relative.w-full, div.item',
    );

    console.log(
      `[Parser] parseSearch() found ${potentialItems.length} potential items`,
    );

    const results = potentialItems
      .map((item) => {
        const titleAnchor =
        ...
        const cover = BatotoParsers.extractImageUrl(item, baseUrl);
      ...
      .filter(
        (m): m is Manga =>
          m !== null && m.id !== '' && m.title !== 'Unknown Title',
      );

    console.log(
      `[Parser] parseSearch() returning ${results.length} valid results`,
    );
    return results;
  }
  ...
  static parseMangaDetails(html: string, baseUrl: string): MangaDetails | null {
    ...
    const title =
      root.querySelector('h3.font-bold')?.textContent?.trim() ||
      root.querySelector('h3')?.textContent?.trim() ||
      'Unknown Title';

    const cover = BatotoParsers.extractImageUrl(root, baseUrl);
    const description =
      root.querySelector('.limit-html')?.textContent?.trim() || '';
    ...
    const chapters = root
      .querySelectorAll('.scrollable-bar > div')
      .map((item) => {
        const anchor =
          item.querySelector("a.link-hover[href*='/chapter/']") ||
  ...
  static parsePopularManga(html: string, baseUrl: string): Manga[] {
    console.log(
      `[Parser] Starting parsePopularManga, HTML length: ${html.length}`,
    );
    const root = parse(html);
    ...
    const cards =
      root.querySelectorAll('div.relative.w-full.group') ||
    ...
    cards.forEach((card, index) => {
      const titleAnchor =
        card.querySelector('a.link-hover') ||
      ...
      const cover = BatotoParsers.extractImageUrl(card, baseUrl);
    ...
    console.log(
      `[Parser] Finished parsePopularManga. Total items: ${allManga.length}`,
    );
    return allManga;
  }
}
