...
import { SyncEngine, type Operation } from '../services/sync';
        ...
        <view className="SyncMonitor">
            <view className="SyncMonitor-header">
                <view className="SyncMonitor-status-row">
                    ...
                    <view className={`SyncMonitor-badge ${isSyncing ? 'syncing' : 'idle'}`}>
                        <text className="SyncMonitor-badge-text">{isSyncing ? 'SYNCING...' : 'IDLE'}</text>
                    </view>
                </view>
            ...
            <scroll-view className="SyncMonitor-list" scroll-y>
                {queue.map((op, i) => (
                    <view key={i} className="SyncMonitor-item">
                        <view className="SyncMonitor-item-header">
                            <text className={`SyncMonitor-op-type ${op.type}`}>{op.type}</text>
                ...
                {queue.length === 0 && (
                    <text className="SyncMonitor-empty">Queue is empty. Local changes synced.</text>
                )}
            </scroll-view>
