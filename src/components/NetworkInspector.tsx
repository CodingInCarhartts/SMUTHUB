...
export function NetworkInspector() {
    ...
    return (
        <view className="NetworkInspector">
                    ...
                    {logs.map((log) => (
                        ...
                        >
                            <view className="NetworkInspector-item-row">
                                <text className={`NetworkInspector-method ${log.method}`}>{log.method}</text>
                                <text className="NetworkInspector-status" style={{ color: log.status && log.status >= 400 ? '#ff5252' : '#4caf50' }}>
                                    {log.status || '...'}
                                </text>
                                <text className="NetworkInspector-duration">{log.duration ? `${log.duration}ms` : ''}</text>
                            </view>
                            <text className="NetworkInspector-url" style={{ textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>{log.url}</text>
                        </view>
                    ))}
                    {logs.length === 0 && (
            ...
            {selectedLog && (
                <view className="NetworkInspector-details-overlay" bindtap={() => setSelectedId(null)}>
                    <scroll-view className="NetworkInspector-details" scroll-y catchtap={() => { }}>
                        <view className="NetworkInspector-details-header">
                            <text className="NetworkInspector-details-title">Request Details</text>
                            <text className="NetworkInspector-close" bindtap={() => setSelectedId(null)}>✕</text>
                        </view>

                        <text className="NetworkInspector-section">General</text>
                        <text className="NetworkInspector-kv">URL: {selectedLog.url}</text>
                        <text className="NetworkInspector-kv">Method: {selectedLog.method}</text>
                        <text className="NetworkInspector-kv">Status: {selectedLog.status} {selectedLog.statusText}</text>
                        <text className="NetworkInspector-kv">Duration: {selectedLog.duration}ms</text>

                        <text className="NetworkInspector-section">Request Headers</text>
                        {selectedLog.requestHeaders && Object.entries(selectedLog.requestHeaders).map(([k, v]) => (
                            <text key={k} className="NetworkInspector-kv">{k}: {v}</text>
                        ))}
                        ...
                        {selectedLog.error && (
                            <>
                                ...
                                <text className="NetworkInspector-kv error">{selectedLog.error}</text>
                            </>
                        )}
                    </scroll-view>
                </view>
