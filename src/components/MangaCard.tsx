...
export function MangaCard({ manga, onSelect, showFavoriteButton = true }: Props) {
  const [isFavorite, setIsFavorite] = useState(StorageService.isFavoriteSync(manga.id));
  ...
  return (
    <view className="MangaCard" bindtap={() => onSelect(manga)}>
      <view className="MangaCard-cover-container">
        ...
        {showFavoriteButton && (
          <view 
            className={isFavorite ? "MangaCard-favorite active" : "MangaCard-favorite"}
